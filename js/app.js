...
const App = {
    ...
    init() {
        ...
        if (localStorage.getItem('bank-loan-darkmode') === 'true') {
    ...
    loadProgress() {
        try {
            const saved = localStorage.getItem('bank-loan-progress');
            if (saved) this.progress = JSON.parse(saved);
            const savedQuiz = localStorage.getItem('bank-loan-quiz-results');
    ...
    saveProgress() {
        localStorage.setItem('bank-loan-progress', JSON.stringify(this.progress));
        localStorage.setItem('bank-loan-quiz-results', JSON.stringify(this.quizResults));
    },
        ...
        let html = `
            <div class="fade-in">
                <div class="dashboard-hero">
                    <h2>銀行融資 Academy へようこそ</h2>
                    <p>銀行融資の基礎から実践まで学べるプラットフォームです。</p>
                </div>
    ...
    toggleDarkMode() {
        ...
        if (isDark) {
            ...
            localStorage.setItem('bank-loan-darkmode', 'false');
        } else {
            document.documentElement.setAttribute('data-theme', 'dark');
            localStorage.setItem('bank-loan-darkmode', 'true');
        }
    },
    ...
    resetProgress() {
        if (confirm('全ての学習進捗をリセットしますか？この操作は元に戻せません。')) {
            this.progress = {};
            this.quizResults = {};
            localStorage.removeItem('bank-loan-progress');
            localStorage.removeItem('bank-loan-quiz-results');
            this.buildSidebar();
            this.updateGlobalProgress();
            this.showDashboard();
        }
